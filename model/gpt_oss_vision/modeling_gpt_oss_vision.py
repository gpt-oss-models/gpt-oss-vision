...
#
# Changes:
# - Multimodal + NoPE integration (VisionAdapter, NoPE per-layer RoPE neutralization)
# - Router vision bias + modality_mask plumbing
# - Updated config import to GPTOSSVisionConfig
...
# Use the vision-enabled config if available
from .configuration_gpt_oss_vision import GPTOSSVisionConfig as GptOssConfig

logger = torch.nn.modules.module.__dict__.get("Logger", None)  # placeholder if needed; not used below


# -----------------------------------------------------------------------------
# Vision adapter (ViT -> projection)
# -----------------------------------------------------------------------------
class VisionAdapter(nn.Module):
    """
    Lightweight ViT-based adapter to produce embeddings compatible with model.hidden_size.
    Tries to import transformers' ViTModel; falls back to a simple conv patch embed if unavailable.
    """
    ...
    def __init__(self, config: GptOssConfig):
        ...
        try:
            ...
            vit_cfg = ViTConfig(
                ...
                intermediate_size=max(4 * config.vision_embed_dim, 1024),
            )
            self.vit = ViTModel(vit_cfg)
        except Exception:
            # Fallback simple patch embed (replace with proper ViT in your repo)
            patch_size = config.vision_patch_size
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        """
        Return: (batch, vision_seq_len, model_hidden_size)
        """
        ...
        if self.vit is not None:
            ...
            x = self.patch_embed(pixel_values)  # (B, C_out, H', W')
            b, c, h, w = x.shape
...
# -----------------------------------------------------------------------------
# Core MoE / attention / rotary implementations (largely preserved)
# -----------------------------------------------------------------------------
...
class GptOssTopKRouter(nn.Module):
    def __init__(self, config):
        ...
        # new: learned bias for vision tokens
        self.vision_bias = nn.Parameter(torch.zeros(self.num_experts))

    def forward(self, hidden_states, modality_mask: Optional[torch.BoolTensor] = None):
        hidden_states = hidden_states.reshape(-1, self.hidden_dim)
        router_logits = F.linear(hidden_states, self.weight, self.bias)  # (seq_len, num_experts)

        # Apply vision bias to vision tokens only
        if modality_mask is not None:
            flat_mask = modality_mask.reshape(-1).to(torch.bool)
            if flat_mask.any():
                router_logits[flat_mask] = router_logits[flat_mask] + self.vision_bias.to(router_logits.dtype)
...
class GptOssMLP(nn.Module):
    ...
    def forward(self, hidden_states, modality_mask: Optional[torch.BoolTensor] = None):
        router_scores, router_indices = self.router(hidden_states, modality_mask=modality_mask)  # (num_experts, seq_len)
...
class GptOssPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            # initialize vision bias to zero
...
class GptOssModel(GptOssPreTrainedModel):
    ...
    def __init__(self, config: GptOssConfig):
        ...
        # VisionAdapter, only instantiated when user enables multimodal in config
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Vision handling: produce vision embeddings and prepend to inputs_embeds
        modality_mask = None
        if self.vision_adapter is not None and pixel_values is not None:
            ...
            if attention_mask is None:
                ...
                # if attention_mask was for text only, prepend ones for vision tokens
        ...
        # Compute base rotary embeddings once per forward
        base_cos, base_sin = self.rotary_emb(hidden_states, position_ids)

        for layer_idx, decoder_layer in enumerate(self.layers):
            # NoPE: neutralize RoPE on periodic layers if enabled
...
class GptOssForCausalLM(GptOssPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        pixel_values: Optional[torch.FloatTensor] = None,  # accept multimodal pixel inputs
        **kwargs: Unpack[TransformersKwargs],
